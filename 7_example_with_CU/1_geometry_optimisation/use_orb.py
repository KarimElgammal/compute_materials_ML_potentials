...
def main():
    ...
    # Fix the surface atoms (those with tag=1)
    constraint_surface = FixAtoms(indices=[i for i, tag in enumerate(atoms.get_tags()) if tag == 1])
    
    # Add a z-axis constraint to the molecule to prevent it from moving too far
    molecule_indices = [i for i, tag in enumerate(atoms.get_tags()) if tag == 0]
    constraint_molecule = FixedPlane(molecule_indices[0], (0, 0, 1))  # Constrain central atom in z
    
    # Combine constraints
    atoms.set_constraint([constraint_surface, constraint_molecule])

    # Set up optimizer with trajectory saving
    opt = BFGS(atoms, trajectory=trajectory_file, logfile=log_file, maxstep=0.1)  # Reduced from 0.2 to 0.1
